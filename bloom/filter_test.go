...
import (
	...
	"github.com/Actinium-project/acmutil/bloom"
)
...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := acmutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(acmutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := acmutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := acmutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
