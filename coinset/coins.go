...
func (cs *CoinSet) TotalValue() (value acmutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue acmutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue acmutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount acmutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue acmutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue acmutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue acmutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue acmutil.Amount, coins []Coin) (Coins, error) {
...
// acmutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() acmutil.Amount {
	return acmutil.Amount(c.txOut().Value)
}
